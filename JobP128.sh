...
parallel --jobs 16 | parallel --pipepart --block $filesize --jobs 16 --sshloginfile \$PBS_NODEFILE --joblog progress.log.\${PBS_JOBID} --workdir $PWD  -a  $INFILE -k ./$SCRIPT > ${INFILE%%.*}.\${PBS_JOBID}.out 
JOBHEAD
